...
void find_connected_region(unsigned char input[SIZEY][SIZEX], int px, int py, unsigned char output[SIZEY][SIZEX]) {
     ...
     */
    int colour = input[py][px];
    output[py][px] = 255; // Mark the current pixel as part of the connected region
    
    // Check for the colour directly of column above
    if (py + 1 < SIZEY && input[py + 1][px] == colour && output[py + 1][px] != 255) {
        find_connected_region(input, px, py + 1, output);
    }
    // Check for the item directly of column below
    if (py - 1 >= 0 && input[py - 1][px] == colour && output[py - 1][px] != 255) {
        find_connected_region(input, px, py - 1, output);
    }
    // Check for the colour directly of row above
    if (px + 1 < SIZEX && input[py][px + 1] == colour && output[py][px + 1] != 255) {
        find_connected_region(input, px + 1, py, output);
    }
    // Check for the colour directly of row below
    if (px - 1 >= 0 && input[py][px - 1] == colour && output[py][px - 1] != 255) {
        find_connected_region(input, px - 1, py, output);
    }
}
